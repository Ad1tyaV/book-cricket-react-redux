...
function FixturesView({
  ...
  const wins = completedTeamMatches.filter(
    (match) => match.winner === selectedTeam
  ).length;
              ...
              {matches.map((match, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell
                      style={{ color: isCompleted ? "#4CAF50" : "#888" }}
                    >
                      {isSimulating ? (
                        <div>
                          <div>Simulating...</div>
      ...
      {activeTab === 1 && (
        ...
        >
          ...
          <p style={{ color: "#bbb", marginTop: 0 }}>
            Remaining matches for {selectedTeam || "selected team"}:{" "}
            {remainingMatchesForSelectedTeam}
          </p>
          <p style={{ color: "#bbb", marginTop: 0 }}>
            Win Ratio: {wins}/{completedTeamMatches.length} ({winRatio}%)
          </p>
          <p style={{ color: "#bbb", marginTop: 0 }}>Losses: {losses}</p>
          <Button
            variant="contained"
            color="primary"
            disabled={
          ...
          {(hasMatchInProgress || isSimulating) && (
            <p style={{ color: "#FFA726", marginBottom: 0 }}>
              Finish current simulation/match before running team-level bulk
              simulation.
            </p>
          )}
          ...
          <h4 style={{ marginTop: 24, marginBottom: 8 }}>
            Top 3 Strike Rates (min 20 balls)
          </h4>
          <Table style={{ backgroundColor: "#171717" }}>
            <TableHead>
              <TableRow>
              ...
              {teamFixtures.length > 0 ? (
                teamFixtures.map((fixture) => (
                  <TableRow key={`fixture-${fixture.index}`}>
                    ...
                    <TableCell
                      style={{
                        color: fixture.result === "-" ? "#aaa" : "#4CAF50",
                      }}
                    >
                      {fixture.result}
                    </TableCell>
