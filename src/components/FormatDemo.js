...
import {
  ...
  PLAYER_ARCHETYPES,
  FORMAT_MODIFIERS,
} from "../helpers/improvedRandomNumber";
...
const FormatDemo = () => {
  ...
  const runSimulation = (format, archetype, balls = 60) => {
    ...
    return {
      outcomes,
      totalRuns,
      strikeRate: ((totalRuns / balls) * 6).toFixed(2),
    };
  };
  ...
  return (
    <div style={{ padding: "20px", color: "white" }}>
                    ...
                    <Grid item xs={4} key={archetype}>
                      ...
                      >
                        ...
                        <div style={{ fontSize: "12px" }}>
                          Wickets: {data.outcomes["-1"]}, Dots:{" "}
                          {data.outcomes["0"]}, 1s: {data.outcomes["1"]}, 2s:{" "}
                          {data.outcomes["2"]}, 4s: {data.outcomes["4"]}, 6s:{" "}
                          {data.outcomes["6"]}
                        </div>
                      </Paper>
                    </Grid>
      ...
      >
        ...
        <ul>
          <li>
            <strong>Format Awareness:</strong> T20 players are more aggressive,
            ODI players more balanced
          </li>
          <li>
            <strong>Dynamic Player Types:</strong> Instead of fixed player
            stats, uses archetypes that adapt
          </li>
          <li>
            <strong>Game Situation:</strong> Considers balls faced, target
            score, wickets in hand
          </li>
          <li>
            <strong>Scalable:</strong> Easy to add new formats (Test, T10, etc.)
            or player types
          </li>
          <li>
            <strong>Realistic:</strong> Strike rates and scoring patterns match
            real cricket
          </li>
        </ul>
      </Paper>
